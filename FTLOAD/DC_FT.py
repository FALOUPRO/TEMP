...
FTMAXLOAD = (45,45,45,45)
K = (2000000, 2000000, 2000000, 2000000)
angle = 0
MCAR = 20
MDRV = 15
rpoint = (0, 0)
...
class DISTANCE:
    ...
    def feetload(mutimass, FC, FT, K, rpoint, angle):
        ...
        DT1 = F1 * 10000/ K[1]
        DT2 = F2 * 10000/ K[2]
        #DTC:对角交点变形位移；CG:第四支腿变形几何位移
        DTC = (DT2-DT0)*DISTANCE.calculate_distance((x0, y0), (XC, YC))/DISTANCE.calculate_distance((x0, y0), (x2, y2))+DT0
        CG = (DTC-DT1)*DISTANCE.calculate_distance((x3, y3), (XC, YC))/DISTANCE.calculate_distance((x1, y1), (XC, YC))+DTC
        ...
        DE1 = FE1 * 10000 / K[1]
        DE2 = FE2 * 10000 / K[2]
        #DEC:对角交点变形位移；CGE:第四支腿变形几何位移
        DEC = (DE2-DE0)*DISTANCE.calculate_distance((A0, B0), (XC, YC))/DISTANCE.calculate_distance((A0, B0), (A2, B2))+DE0
        CGE = (DEC-DE1)*DISTANCE.calculate_distance((A3, B3), (XC, YC))/DISTANCE.calculate_distance((A1, B1), (XC, YC))+DEC
        if CG <=0:#第四支腿有几何位移（压缩量）时
            F3 = 0
            print('FK=', CG, DTC, DT0, DT1, DT2, F0, F1, F2, F3)
        elif CG > 0:
            ...
            Dp = np.linalg.det(np.array([[x0, x1, x2, x3], [y0, y1, y2, y3], [1, 1, 1, 1], [A, B, C, D]]))
            ...
            F2 = D2 / Dp
            F3 = D3 / Dp
            print('k=', CG, DTC, DT0, DT1, DT2, F0, F1, F2, F3, K[0], K[1], K[2], K[3])

        if CGE <=0:
            FE3 = 0
            print('FKE=', CGE, DEC, DE0, DE1, DE2, FE0, FE1, FE2, FE3)
        elif CGE > 0:
            ...
            FE2 = R2 / R
            FE3 = R3 / R
            print('ke=', CGE, DEC, DE0, DE1, DE2, FE0, FE1, FE2, FE3)
    ...
    def FTLOAD(mutimass, tol, tolerance, FT, K, rpoint, angle):
        ...
        DS1 = (x-x3)/(x1-x3)*(y1-y3)+y3
        DS2 = (x-x4)/(x2-x4)*(y2-y4)+y4
        FC = 0
        if y >= DS1 and y >= DS2:
            ...
            FC = 41
        (F1, F2, F3, F4) = DISTANCE.feetload(mutimass, FC, FT, K, rpoint, angle)[0]
        return (F1, F2, F3, F4), (x, y), G, angle
...
for angle in np.arange(start_angle, end_angle + deta, step_size):
    ...
    (F1, F2, F3, F4),(C1, C2),DLOAD = DISTANCE.FTLOAD(DISTANCE.mutimass(LOAD, rpoint, loadpoint, MCAR, MCARpoint, MDRV, MDRVpoint, angle), tol, tolerance, FT, K, rpoint, angle)[0:3]
    ftload = [F1, F2, F3, F4]
    # 将 MAXLOAD 值添加到列表中
    ...
    case_values.append(C1)
    mr_values.append(C2)
    lr_values.append(DLOAD)
    x0_values.append(F1)
    y0_values.append(F2)
    x1_values.append(F3)
    y1_values.append(F4)
...
# 绘制极坐标图形，注意角度需要转换为弧度
ax_polar.plot(np.radians(result_df['Angle']), result_df['FTLOAD'].apply(lambda x: pd.Series(x)))
# 增加新的曲线，使用黑色显示
ax_polar.plot(np.radians(result_df['Angle']), result_df['MAXLOAD'], color='black')
# 设置标题
ax_polar.set_title('FTLOAD', va='bottom')
